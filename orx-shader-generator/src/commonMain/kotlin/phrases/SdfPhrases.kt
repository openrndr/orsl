    ...
    val sdSolidAngle = """float sdSolidAngle( vec3 p, vec2 c, float ra )
{
  ...
  return (s<0.0) ? length(q)-r :
    ...
    @ShaderPhrase
    val sdCutHollowSphere = """float sdCutHollowSphere( vec3 p, float r, float h, float t ) {
  // sampling independent computations (only depend on shape)
  float w = sqrt(r*r-h*h);
  ...
  return ((h*q.x<w*q.y) ? length(q-vec2(w,h)) : 
    ...
    @ShaderPhrase
    val sdDeathStar = """float sdDeathStar( vec3 p2, float ra, float rb, float d ) {
  // sampling independent computations (only depend on shape)
  ...
  if( sign(y)*a2*y2<k ) return  sqrt(x2 + y2)        *il2 - r1;
    ...
    @ShaderPhrase
    val sdEllipsoid = """float sdEllipsoid( vec3 p, vec3 r ) {
  float k0 = length(p/r);
  ...
  return k0*(k0-1.0)/k1;
    ...
    @ShaderPhrase
    val sdRhombus = """float sdRhombus( vec3 p, float la, float lb, float h, float ra ){
  p = abs(p);
  vec2 b = vec2(la,lb);
  ...
  return min(max(q.x,q.y),0.0) + length(max(q,0.0));
    ...
    @ShaderPhrase
    val sdOctahedron = """float sdOctahedron( vec3 p, float s ) {
  p = abs(p);
  ...
  return length(vec3(q.x,q.y-s+k,q.z-k)); 
    ...
    @ShaderPhrase
    val sdOctaHedronBound = """float sdOctahedronBound( vec3 p, float s) {
  p = abs(p);
  return (p.x+p.y+p.z-s)*0.57735027;
}"""

    @ShaderPhrase
    val sdPyramid = """float sdPyramid( vec3 p, float h )
{
  float m2 = h*h + 0.25;
